        ...
        <div>
            <h1 class="text-2xl font-bold flex items-center gap-3">
                ...
                <span class="text-xs bg-red-100 text-red-600 px-3 py-1 rounded-full">
                    Total {{ $spp->total() }} Data
                </span>
            </h1>
            <p class="text-sm text-gray-500 mt-1">
                Kelola data SPP berdasarkan tahun dan nominal
            </p>
        </div>

        <div class="flex gap-2 mt-4 md:mt-0">
            <a href="/admin/spp/tambah"
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-100 text-gray-600 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4 text-center">Bantuan</th>
...
<td class="px-6 py-4 text-center">
    <div class="flex justify-center items-center gap-2">
        <!-- EDIT -->
        <a href="/admin/spp/{{ $item->id_spp }}/edit"
           title="Edit"
        ...
        <!-- DELETE -->
        <form action="/admin/spp/{{ $item->id_spp }}"
              method="POST">
            @csrf
            @method('DELETE')
            <button type="button"
...
<div class="flex flex-col md:flex-row md:justify-between md:items-center mt-6 text-sm gap-3">
    ...
    <p class="text-gray-500">
        Menampilkan
        <span class="font-medium">{{ $spp->firstItem() }}</span>
        –
        <span class="font-medium">{{ $spp->lastItem() }}</span>
        dari
        <span class="font-medium">{{ $spp->total() }}</span>
        data SPP
    </p>
    ...
    <div class="flex gap-1">
        ...
        @else
            <a href="{{ $spp->previousPageUrl() }}"
               class="px-3 py-1 border rounded hover:bg-gray-100">«</a>
        @endif

        {{-- Page Numbers --}}
        @foreach ($spp->getUrlRange(1, $spp->lastPage()) as $page => $url)
            @if ($page == $spp->currentPage())
    ...
    <p class="text-center text-sm text-gray-500 mt-8">
        © 2026 EduPay — Manajemen Data SPP
    </p>
        ...
        html: `
            <div class="text-sm text-gray-600 leading-relaxed">
                <p class="mb-2">
                    ...
                    <span class="font-semibold text-red-600">data SPP</span> ini?
                </p>
